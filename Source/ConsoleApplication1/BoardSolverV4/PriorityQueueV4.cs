...
{
    internal class PriorityQueueV4
    {
        // https://en.wikipedia.org/wiki/Binary_heap
        private readonly List<NodeV4> binaryHeap = new List<NodeV4>();

        private readonly HashSet<NodeV4> hashSet = new HashSet<NodeV4>();

        public bool Contains(NodeV4 node)
        {
            if (node == null)
            {
        ...
        public void Enqueue(NodeV4 node)
        {
            if (node == null)
            {
                throw new ArgumentNullException(nameof(node));
        ...
        public NodeV4 Dequeue()
        {
            ...
            NodeV4 minNode = this.binaryHeap[0];
            NodeV4 lastNode = this.binaryHeap[this.binaryHeap.Count - 1];
